...
var sumTotal = function (products) {
  return _.reduce(products, function(memo, product) {
    return memo + Number(product.price.replace('$', ''));
  }, 0);
...
var dessertCategories = function(desserts) {
  return _.reduce(desserts, function (dessertTypes, dessert) {
    dessertTypes[dessert.type] = (dessertTypes[dessert.type] === undefined) ? 1
...
var applyCoupon = function(groceries, coupon) {
  return _.map(groceries, function (grocery) {
    groceryPrice = Number(Number(grocery.price.replace('$', ''))
    .toFixed(2)
    .toString()
    .replace('.', ''));
    grocery['salePrice'] = '$' + ((groceryPrice * (1 - coupon)) / 100).toFixed(2).toString();
    return grocery;
  });
};
